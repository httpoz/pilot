    ...
    class PilotPostType
    {
        ...
        public function create($options)
        {
            $options['post_type_name'] = function () use ($options) {
                $labels = [
                  'name'               => _x(ucfirst($options['plural']),
                    'post type general name', 'pilot'),
                  'singular_name'      => _x(ucfirst($options['singular']),
                    'post type singular name', 'pilot'),
                  'menu_name'          => _x(ucfirst($options['plural']),
                    'admin menu', 'pilot'),
                  'name_admin_bar'     => _x(ucfirst($options['plural']),
                    'add new on admin bar', 'pilot'),
                  'add_new'            => _x('Add New', $this->post_type,
                    'pilot'),
                  'add_new_item'       => __('Add ' . ucfirst($options['singular']),
                    'pilot'),
                  'new_item'           => __('New ' . ucfirst($options['singular']),
                    'pilot'),
                  'edit_item'          => __('Edit ' . ucfirst($options['singular']),
                    'pilot'),
                  'view_item'          => __('View ' . ucfirst($options['singular']),
                    'pilot'),
                  'all_items'          => __('All ' . ucfirst($options['plural']),
                    'pilot'),
                  'search_items'       => __('Search ' . ucfirst($options['plural']),
                    'pilot'),
                  'not_found'          => __('No ' . ucfirst($options['plural']) . ' found.',
                    'pilot'),
                  'not_found_in_trash' => __('No ' . ucfirst($options['plural']) . ' found in Trash.',
                    'pilot'),
                  'parent_item_colon'  => '',
                ];
                $args   = [
                  ...
                  'supports'            => ($options['supports']) ? $options['supports'] : [
                    'title',
                    'editor',
                    'thumbnail',
                  ],
